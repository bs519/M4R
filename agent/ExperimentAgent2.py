...
class ExperimentAgent2(TradingAgent):
    """
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if currentTime >= self.mkt_open + self.init_wakeup_time and not self.sim_check:
            ...
            try:
                journal = Journal.fromFile(f"Results/experiment1.2/experiment1.2.2_{self.m}.jrnl")
                ...
                # constant to set history and simulated orderbooks in inference to the same length
                n = history[0][0][-1, 0]
                #time the inference
                start_time = time.time()

                model, journal = ExperimentAgent2.infer(history, n)

                end_time = time.time()
                print("time taken:", end_time - start_time)

                # save the final journal file, depending on the experiment
                journal.save(f"Results/experiment1.2/experiment1.2.2_{self.m}.jrnl")
            ...
            def run_simulation(i):
                ...
                while True:
                    try:
                        # +self.sim_time} "
                        subprocess.run([f"python3 -u abides.py -c bap -t ABM -d {self.historical_date} {self.startsimTime} {self.endsimTime} -l inference_agent_exp1.2.2 -n {num_noise} -m {num_momentum} -a {num_value} -z {self.starting_cash} -r {self.r_bar} -g {self.sigma_n} -k {self.kappa} -b {self.lambda_a}"], shell=True)
                    except UnboundLocalError:
                        print("Simulation error, we will try again")
                        continue
                    else:
                        processed_orderbook =  make_orderbook_for_analysis(f"log/inference_agent_exp1.2.2/EXCHANGE_AGENT.bz2", f"log/inference_agent_exp1.2.2/ORDERBOOK_ABM_FULL.bz2", num_levels=1,
